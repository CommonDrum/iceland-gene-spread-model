...
class GraphInterface():
    ...
    def __init__(self):
        ...
        self.weights_infection = [0.01, 0.99]  # probability of being infected

        self.no_of_children = [0, 1, 2, 3]
        self.weights_children = [0.3, 0.2, 0.4,
                                 0.1]  # probability of having a child depending on the number of children

        self.capacity = 15000  # HEREEEEEEEEEEEEEEEE

        self.sex = ['F', 'M']
        self.sex_weights = [0.5, 0.5]  # probability of having a child depending on the number of children
        # reproduction rate
        self.reproduction_rate = [0.38, 0.31, 0.14, 0.03, 0.02, 0, 0, 0,
                                  0]  # probability of having a child depending on the number of children
        ...
        self.age_prefered_region = ['none', "urban", "urban", "rural", "rural", "rural", "rural", "rural", "rural",
                                    "rural"]
        ...
        self.region_infected_population = [0,0,0,0,0,0]
    ...
    def new_node(self, age=0, is_infected=False, partner=0, family=[], no_of_children=0, region=""):
        ...
        self.G.add_node(id, age=age, sex=sex, is_infected=is_infected, partner=partner, no_of_children=no_of_children,
                        region=region)
        # connect the family
    ...
    def partner_node(self, node):
        if self.G.nodes[node]['partner'] == 0:
            for neighbor in self.G.neighbors(node):
                if self.G.nodes[neighbor]['partner'] == 0 and self.G.nodes[neighbor]["sex"] != self.G.nodes[node][
                    "sex"]:
    ...
    def relocate_node(self, node):
        ...
        region_index = self.regions.index(self.G.nodes[node]['region'])
        if random.random() < self.relocation_rate[region_index] * self.age_relocation_modifier[
            self.G.nodes[node]["age"]]:
    ...
    def region_infected(self, node):
        region_index = self.regions.index(self.G.nodes[node]['region'])
        self.region_population[region_index] += 1
    
    def reset_region_population(self):
        self.region_infected_population = [0, 0, 0, 0, 0, 0]
    ...
    def step(self):
        ...
        for node in list(self.G.nodes):
            ...
            self.age_node(node)
            # check if node is infected and its region
        for node in list(self.G.nodes):
            if self.G.nodes[node]['is_infected']:
                region_index = self.regions.index(self.G.nodes[node]['region'])
                self.region_infected_population[region_index] += 1
...
if __name__ == "__main__":
    ...
    for i in range(5):
        ...
        for j in tqdm(range(iteration_size)):
            ...
            region_infected_pop.append(G.region_infected_population)

            if G.infected == 0:
                break

        plt.figure("1")
        plt.plot(populaion_list, label="Population")
        plt.plot(infected_list, label="Mutated population")
        ...
        # Write the data to the CSV file
            ...
            writer.writerow("Southwest_Infected:")
            southwest_infected = [x[0] for x in region_infected_pop]
            writer.writerow(southwest_infected)
            writer.writerow("Reykjavik North_Infected:")
            Reykjavik_North = [x[1] for x in region_infected_pop]
            writer.writerow(Reykjavik_North)
            writer.writerow("Reykjavik South_Infected:")
            Reykjavik_South = [x[2] for x in region_infected_pop]
            writer.writerow(Reykjavik_South)
            writer.writerow("South_Infected:")
            South = [x[3] for x in region_infected_pop]
            writer.writerow(South)
            writer.writerow("Northeast_Infected:")
            Northeast = [x[4] for x in region_infected_pop]
            writer.writerow(Northeast)
            writer.writerow("Northwest_Infected:")
            Northwest = [x[5] for x in region_infected_pop]
            writer.writerow(Northwest)
